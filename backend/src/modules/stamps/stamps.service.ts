...
import { triggerReferralBonus } from '../referrals/referrals.service.js';
...
export async function issueStamps(
  ...
  // Dedicated referral trigger — fires once when the referred customer
  // receives their very first stamp ever. Isolated in referrals.service.ts.
  if (customer.lifetime_stamps === 0 && input.amount > 0) {
    triggerReferralBonus(businessId, input.customerId, stampCount).catch((err) =>
      logger.error({ err }, 'Referral bonus trigger failed'),
    );
  }
